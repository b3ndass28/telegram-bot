...
def create_social_card(account, stats, count=3):
    ...
    draw.text((75, 70), f"{platform_label(account.get('platform'))} @{account.get('username')}", fill=text, font=font_big)
    draw.text((75, 132), f"Last {count} video{'s' if count != 1 else ''} summary", fill=muted, font=font)
    ...
    if not videos:
        ...
        for v in videos:
            pot = v.get("potential", "Low")
            pot_color = green if pot == "High" else yellow if pot == "Medium" else red
            ...
            draw.text((855, y + 18), f"Potential {pot}", fill=pot_color, font=font)
            draw.text((105, y + 55), f"Comments {k_format(v.get('comments'))}   Shares {k_format(v.get('shares'))}", fill=muted, font=font_small)
            y += 112
