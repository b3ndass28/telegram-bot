...
def cancel_download_kb(uid): return InlineKeyboardMarkup([[InlineKeyboardButton('❌ Отменить скачивание', callback_data=f'cancel_download:{uid}')]])
...
def reminder_kb(item):
    rows = [
        ...
        [InlineKeyboardButton('⏱ 1ч', callback_data=f'post_reminder_set:1h:{item}'),
         InlineKeyboardButton('⏱ 2ч', callback_data=f'post_reminder_set:2h:{item}'),
         InlineKeyboardButton('⏱ 3ч', callback_data=f'post_reminder_set:3h:{item}'),
         InlineKeyboardButton('⏱ 4ч', callback_data=f'post_reminder_set:4h:{item}')],
        [InlineKeyboardButton('⏱ 6ч', callback_data=f'post_reminder_set:6h:{item}'),
         InlineKeyboardButton('⏱ 8ч', callback_data=f'post_reminder_set:8h:{item}'),
         InlineKeyboardButton('⏱ 12ч', callback_data=f'post_reminder_set:12h:{item}'),
         InlineKeyboardButton('⏱ 24ч', callback_data=f'post_reminder_set:24h:{item}')]
    ]
    for s in range(1, 16, 5):
        rows.append([InlineKeyboardButton(f'{d}д', callback_data=f'post_reminder_set:{d}d:{item}') for d in range(s, min(s+5, 16))])
    rows.append([InlineKeyboardButton('🗑 Убрать напоминание', callback_data=f'post_reminder_remove:{item}')])
    rows.append([InlineKeyboardButton('⬅️ Назад', callback_data=f'post_back:{item}')])
    return InlineKeyboardMarkup(rows)
...
async def reset_cmd(update, context):
    if not await require_access(update, context): return
    uid=update.effective_user.id; context.application.bot_data.pop(f'processing_{uid}',None); context.application.bot_data.pop(f'download_task_{uid}',None); context.application.bot_data.pop(f'queued_{uid}',None)
...
async def post_action_callback(update, context):
    ...
    if d.startswith('post_reminder_set:'):
        _,rk,item=d.split(':',2)
        if rk=='test':
            seconds=60; label='🧪 Тест — 1 минута'
        elif rk.endswith('h'):
            hours=int(rk[:-1]); seconds=hours*3600; label=f'⏰ Через {hours} ч.'
        else:
            days=int(rk[:-1]); seconds=days*86400; label=f'⏰ Через {days} дн.'
        due_ts=time.time()+seconds
        add_reminder(item, q.from_user.id, due_ts, label)
        it=update_item(item, {'reminder':rk,'reminder_label':label,'reminder_due_ts':due_ts})
